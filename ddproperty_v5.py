...
property_type = {
    ...
    'condo_rent'    : {'type_id': 2, 'type_value': 'N', 'route': 'CONDO', "property_sell_rent": 'รวมประกาศให้เช่า', 'listing_type': 'rent', 'start': 1, 'end': 75},
    'townhouse_rent': {'type_id': 3, 'type_value': 'T', 'route': 'TOWN', "property_sell_rent": 'รวมประกาศให้เช่า', 'listing_type': 'rent', 'start': 1, 'end': 30},
}
# +++++++++++++ แก้ข้อมูลเพื่อเก็บข้อมูล +++++++++++++ #
...
def get_detail(prop_url: str, type_id: int, ID: int):
    ...
    if prop_url:
        ...
        if next_data_script != "content_not_found":
            ...
            _error_count += 1
            if(_error_count >= 10):
                print("❗ เกิดข้อผิดพลาดมากเกินไปในการดึงข้อมูลจาก URL:", prop_url)
                _error_count = 0
                return
