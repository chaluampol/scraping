...
def get_data(prop_url, type_id, ID ):
    ...
    try:
        ...
        try:
            _addresss = soup.find('td', class_='pd_detail_title1').parent.parent.get_text().split('ที่อยู่:')[1].split('เนื้อที่ดิน(ตร.วา):')[0].replace(' ','')
            _locationss = soup.find('td', class_='pd_detail_title1').parent.parent.get_text().split('ที่อยู่:')[1].split('เนื้อที่ดิน(ตร.วา):')[0]
            _province_codes = _locationss.split()[6].replace('จังหวัด','')
            _district_codes = _locationss.split()[5].replace('อำเภอ','')
            _sub_district_codes = _locationss.split()[4].replace('ตำบล','')
...
if __name__ == '__main__':
   ...
   print('Export', len(ids), web, 'Rows To CSV File Completed!!!! ')
